...
let getPossibleFourNumberCombinationsAt x y = seq {
    // Horizontal...
    if x < width-4 then
    ...
    if x < width-4 && y < height-4 then
        yield [| grid.[y].[x]; grid.[y+1].[x+1]; grid.[y+2].[x+2]; grid.[y+3].[x+3] |]
    // Diagonal up and right...
    if x < width-4 && y >= 3 then
        yield [| grid.[y].[x]; grid.[y-1].[x+1]; grid.[y-2].[x+2]; grid.[y-3].[x+3] |]
}

let allPositionsOnGrid = seq {
...
let answer =
    allPositionsOnGrid
    |> Seq.map (fun a -> getPossibleFourNumberCombinationsAt (fst a) (snd a) |> Seq.toList)
    |> Seq.toList
    |> List.concat
    |> Seq.map (Seq.fold (fun acc elem -> acc * elem) 1)
    |> Seq.max
